...
if ( ! class_exists( 'frontend_text_edit_Updater_Notice', false ) ) {
	/** One notice for all participating plugins, without an updater dependency. */
	final class frontend_text_edit_Updater_Notice {
		...
		private static function action() {
			...
			if ( isset( $installed[ $updater ] ) ) {
				...
				return array(
					...
					'label' => __( 'Activate Devenia MCP Updater', 'frontend-text-edit' ),
					'installed' => true,
				);
			}
			...
			return array(
				...
				'label' => __( 'Install Devenia MCP Updater', 'frontend-text-edit' ),
				'installed' => false,
			);
		}
		...
		public static function links( $links, $plugin_file ) {
			...
			if ( $action ) {
				...
				if ( ! $action['installed'] ) {
					$links['devenia-updater-download'] = '<a href="' . esc_url( 'https://downloads.devenia.com/devenia-mcp-updater.zip' ) . '">' . esc_html__( 'Download updater ZIP', 'frontend-text-edit' ) . '</a>';
				}
			}
			return $links;
		}
		...
		public static function render() {
			...
			echo esc_html__( 'Get updates for your Devenia plugins through WordPress. Devenia MCP Updater connects them to the update channel; you choose which plugins update automatically.', 'frontend-text-edit' );
			echo '</p><p>';
			if ( ! $action['installed'] ) {
				echo '<a href="' . esc_url( 'https://downloads.devenia.com/devenia-mcp-updater.zip' ) . '">' . esc_html__( 'Download the updater ZIP', 'frontend-text-edit' ) . '</a> — ';
				echo esc_html__( 'then upload it using the install button.', 'frontend-text-edit' ) . ' ';
			}
			...
			echo '<button type="submit" class="button-link">' . esc_html__( 'Dismiss this reminder', 'frontend-text-edit' ) . '</button>';
			echo '</p></form></div>';
		}
		...
		public static function dismiss() {
			...
			if ( ! current_user_can( 'install_plugins' ) && ! current_user_can( 'activate_plugins' ) ) {
				wp_die( esc_html__( 'You do not have permission to manage plugins.', 'frontend-text-edit' ) );
			}
